...
void swap_elements(long long int* first_element, long long int* second_element) {
	...
	for (int i = 0; i < array_length - 1; i++) {
		for (int j = 0; j < array_length - i - 1; j++) {
			if (array_of_numbers[j] > array_of_numbers[j + 1]) {
				swap_elements(&array_of_numbers[j], &array_of_numbers[j + 1]);
			}
		}
...
//проверка, содержит ли аргумент в начале --to= или --from=
...
//определение, каким типом (--from= или --to=) является аргумент
//на вход подаются только корректные аргументы
...
void read_array(long long int* array, int* count_of_elements) {
	//в переменную c будем считывать символ после числа, когда он станет равным \n прекратим
	char c;
	int i = 0;
...
/*распределение чисел по трем массивам: числа, меньшие или равные from,
числа, большие или равные to и все остальные*/
void distribute_numbers(long long int* array_of_numbers, long long int* small_elements, long long int* normal_elements,
	...
	for (int i = 0; i < count_of_elements; i++) {
		...
		if (array_of_numbers[i] > from && array_of_numbers[i] < to) {
...
int main(int argc, char* argv[]) {
	...
	int count_of_normal_elements;
	//количество элементов, изменивших свою позицию после сортировки
	...
	//поиск количества элементов, изменивших позицию
